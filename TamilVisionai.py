...
def wishings():
    ...
    if 0 <= hour < 12:
        print("Vision: காலை வணக்கம்")
        speak("Kaalai vanak am!")
    elif 12 <= hour < 18:
        print("Vision: பிற்பகல் வணக்கம்...")
        speak("pirpagal vanak am!")
    else:
        print("Vision: மாலை வணக்கம்")
        speak("Maalai Vanakam!")
    print("Vision: என் பெயர் விஷன்")
    speak("en peyar vision")
    print("Vision: நான் உங்களுக்கு என்ன உதவி செய்ய வேண்டும்")
    speak("thangalluku enna vaendum")
class TamilVisionAssistant:
    def commands(self):
        r = sr.Recognizer()
        with sr.Microphone() as source:
            print("Vision: கேட்கிறேன் ...")
            r.pause_threshold = 1
            r.adjust_for_ambient_noise(source, duration=1)
            audio = r.listen(source)
        try:
            print("Vision: ஒரு சில நேரம் காத்திருக்கவும் Sir...")
            query = r.recognize_google(audio, language='Ta-in')
            print(f"Vision: You Just Said: {query}\n")
        except:
            print("Vision: தயவுசெய்து மீண்டும் கூறுங்கள்...")
            speak("thayava seithu meendum koorungal ")
            query = "none"
        return query.lower()
    ...
    def screenshot(self):
        ...
        print("திரைபிடிப்பு எடுக்குதல்")
        speak("thiraipidipu eduthal")
    def joke(self):
    ...
    def run(self):
        ...
        while True:
            ...
            elif 'ஸ்கிரீன் க்ளோஸ் பண்ணு' in query:
                pyautogui.hotkey('alt', 'f4')

            elif 'வெளியே போ' in query:
                pyautogui.hotkey('win', 'd')

            elif 'மூடு' in query:
                self.getlost()
            elif "திரையை படி" in query:
...
if __name__ == '__main__':
    while True:
        try:
            assistant = TamilVisionAssistant()
            assistant.run()
        except Exception as e:
